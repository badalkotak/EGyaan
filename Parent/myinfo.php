...
session_start();
$user=$_SESSION['uname'];
$result = mysqli_query($con,"SELECT * FROM `Student` WHERE `ParentEmail`='$user'");
...
$b=$row['BatchId'];
$getBatchName=mysqli_query($con,"SELECT BatchName FROM Batch WHERE BatchId='$b'");
			while($batchrow=mysqli_fetch_array($getBatchName))
			{
				$bname=$batchrow['BatchName'];
			}
$uid=$row['StudentId'];
}
echo "<h4><label>Batch: $bname</label><br>";
echo "<label>Student Name: $stud_usr</label><br>";
echo "<label>Firstname: $pfname</label><br>";
echo "<label>Lastname: $lname</label><br>";
echo "<label>Student Mobile Number: $mn</label><br>";
echo "<label>Parent Mobile Number: $pmn</label><br>";
echo "<label>Student's Email ID: $semail</label><br>";
echo "<label>Parent's Email ID: $pemail</label><br></h4>";
?>
        </section><!-- /.content -->
      </div><!-- /.content-wrapper -->
      <!-- Main Footer -->
