...
$b_name=mysqli_query($con,"Select BatchName From Batch Where BatchId='$BatchId'");
while($brow=mysqli_fetch_array($b_name))
{
	$br_name=$brow['BatchName'];
}
  $course_uid=$row['CourseId'];
echo '<div class="col-md-12">
              <div class="box box-default collapsed-box box-solid">
                <div class="box-header with-border">';
                  echo "<h3 class=box-title>$course- $br_name</h3>";
...
<tbody>';
            $doubt=mysqli_query($con,"SELECT * FROM `Doubt` WHERE `CourseId`='$course_uid'");
        while($db_row=mysqli_fetch_array($doubt)){$title=$db_row['Question'];$id=$db_row['Doubtid'];$ans=$db_row['Answer'];
      $i++;
  echo "<tr>";
echo "<td>";
echo $i;
...
echo "</td>";
echo "<td>";
echo "<form action=ans_doubt.php method=post><button type=submit class=btn btn-success name=give value=$id>Give Answer</button></form>";
echo "</td>";
echo "<td>";
echo "<form action=view_doubt.php method=post><button type=submit class=btn btn-success name=view value=$id>View Answer</button></form>";
echo "</td>";
echo "<td>";
if($ans=="")
  echo "Pending";
...
echo "</td>";
echo "<td>";
echo "<form action=del_doubt.php method=post><button type=submit class=btn btn-success name=del value=$id onclick=myFunction()>Delete</button></form>";
echo "</td>";
echo "</tr>";
}
echo "</tbody></table>";
