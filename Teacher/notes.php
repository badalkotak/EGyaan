...
$result = mysqli_query($con,"SELECT * FROM `Notes` where CourseId='$courseId'");
...
echo "</td>";
echo "<td>";
echo "<form action=del_notes.php method=post><button type=submit class=btn btn-default name=del value='$uid' onclick=myFunction() >Delete</button></form>";
echo "</td>";
echo "</tr>";
}
}
...
?>
         ...
         <div class="col-md-4">
              <div class="box box-default collapsed-box box-solid">
                ...
                <div class="box-body">
                  <form action=add_notes.php role="form" method="post" enctype="multipart/form-data">
			...
			$b_id=$c_row['BatchId'];
			$getBatchName=mysqli_query($con,"SELECT BatchName FROM Batch WHERE BatchId='$b_id'");
			while($batchrow=mysqli_fetch_array($getBatchName))
			{
				$b_name=$batchrow['BatchName'];
			}
                    }

                      
                      echo "<option value=$courseId>$course_name- $b_name</option>";
                      }
                      echo "</select>";
                    }
                    else
        ...
        <div class="form-group">
          ...
          <input type="text" class="tb5" name="title" placeholder="Enter Title">
        </div>
        <div class="form-group">
          <label for="file">Upload Book: </label>
          <input type="file" class="tb5" name="file" enctype="multipart/form-data">
        </div>
        <?php echo "<button type=submit class=btn btn-default name=i value=$k >Add Notes</button>";?>
      </form><!-- /.box-body -->
              </div><!-- /.box -->
            </div><br><br><br><!-- /.col -->
        </section>
