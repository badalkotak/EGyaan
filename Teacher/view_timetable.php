...
for($i=0;$i<$s;$i++)
{
  for($j=0;$j<=7;$j++)
  {
    $n="r".$i."c".$j;
    //echo "<td>";
$result_t = mysqli_query($con,"SELECT * FROM Timetable where BatchId='$BatchId' && CellNo='$n'");
...
for($i=0;$i<$s;$i++)
{
  for($j=0;$j<=7;$j++)
  {
    $n="r".$i."c".$j;
    echo "<td>";
$get = mysqli_query($con,"SELECT * FROM `Timetable` WHERE `BatchId`='$BatchId' && `CellNo`='$n'");
