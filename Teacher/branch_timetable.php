...
<label for="menu1">Select Batch:</label><br>
<select class="tb5" name="branch" onchange="showTimetable(this.value)">
  <option value="">Select Batch..</option>
<?php
include("dbcon.php");
$result = mysqli_query($con,"SELECT BatchId FROM Teacher where TeacherId='$TeacherId'");
while($row = mysqli_fetch_array($result))
{
$BatchId=$row['BatchId'];
$getBatchName=mysqli_query($con,"SELECT BatchName FROM Batch WHERE BatchId='$BatchId'");
			while($batchrow=mysqli_fetch_array($getBatchName))
			{
				$bname=$batchrow['BatchName'];
			}
echo "<option value=$BatchId>$bname</option>";
}
?>
</select><br><br>
